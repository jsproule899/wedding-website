...
function App() {
  ...
  return (
    <>
      <img src={backgroundImgTopLeft} alt="flowery background" className='fixed top-0 left-0 pr-[50%] -z-10 ml-auto animate-grow-top-left' />
      <img src={backgroundImgTopRight} alt="flowery background" className='fixed top-0 right-0 pl-[50%] -z-10 ml-auto animate-grow-top-right' />
      <NavBar />
      <div id='Home' className="card max-w-full">
        <h1 className='text-7xl sm:text-9xl [word-spacing:99999px] mb-10 animate-fade-in '>We're Getting Married</h1>
        {guestOne && (<h2 className='text-4xl sm:text-6xl m-8 capitalize animate-[fade-in_3s]'>{guestOne}{guestTwo && family ? `, ${guestTwo}` : guestTwo && ` & ${guestTwo}`} {plusOne && " & Plus One"} {family && " & Children"} </h2>)}
        ...
        <h2 className='text-4xl sm:text-5xl m-3 animate-[fade-in_2s]'>Sunday 9<sup>th</sup> August 2026</h2>
        <hr className='w-sm sm:w-lg border mx-auto' />
        <p className='text-4xl mt-5'>
          {daysTogether} Days Together...
        </p>
        <p className='text-4xl mb-5'>
          {daysLeft} Days until we say I Do!
        </p>
      ...
      <div id='Travel' className='my-50 flex justify-center'>
      ...
      <img src={backgroundImgBottomLeft} alt="flowery background" className='fixed bottom-0 lef-0 pr-[50%] -z-10  animate-grow-bottom-left' />
      <img src={backgroundImgBottomRight} alt="flowery background" className='fixed bottom-0 right-0 pl-[50%] -z-10 animate-grow-bottom-right' />
